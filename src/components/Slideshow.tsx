      ...
      <div className="relative w-full h-full">
        {images.map((image, index) => {
          ...
          if (isCurrent) {
            ...
            if (direction === 'next' && isPrev) {
              slideClass = 'animate-slideOutToLeft';
            } else if (direction === 'prev' && isNext) {
              slideClass = 'animate-slideOutToRight';
            }
          }
          ...
          return (
            <div
              key={index}
              className={`absolute inset-0 w-full h-full transition-opacity duration-600 ${slideClass}`}
            >
              <img
                src={image}
                alt={`Slide ${index + 1}`}
